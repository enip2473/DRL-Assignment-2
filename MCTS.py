...
def generate_best_move(board):
    ...
    if move == 1:
        for cell in empty:
            if not has_neighbor(board, cell):
                continue
            r, c = cell
            board[r][c] = 1
        ...
        for move1, move2 in itertools.combinations(empty, 2):
            r1, c1 = move1
            r2, c2 = move2
            if not has_neighbor(board, move1) or not has_neighbor(board, move2):
                continue
            board[r1][c1] = 1
            board[r2][c2] = 1
            score = evaluate_board(board)
...
def select_move_rule(board, color):
    if color == "W":
        board_cp = board.copy()
        for c in range(BOARD_SIZE):
            for r in range(BOARD_SIZE):
        ...
        return select_move_rule(board_cp, "B")
...
def select_move(board, color):
    color = 1 if color == "B" else 2
    mcts = MCTS(iterations=50)
    root = MCTS_Node(state=board, color=color)
...
class MCTS_Node:
    def __init__(self, state, color, action=None, parent=None):
        """
        state: current board state (numpy array)
        color: whose turn is it (1 for black, 2 for white)
        ...
        empty_cells = get_empty_cells(self.state)
        self.untried_actions = [cell for cell in empty_cells if has_neighbor(self.state, cell)]
        if np.sum(self.state) == 0:
            self.untried_actions.append((5, 5))
...
class MCTS:
    ...
    def select_child(self, node, board):
        ...
        r, c = best_node.action
        board[r][c] = best_node.color
        return best_node
    ...
    def backpropagate(self, node, value):
        while node is not None:
            ...
            if node.parent is not None and node.parent.color != node.color:
                value = -value
            node = node.parent
    ...
    def expand(self, node, board):
        ...
        r, c = action
        board[r][c] = node.color
        count_1, count_2 = count_moves(board)
        child_color = node.color if count_1 == count_2 else 3 - node.color
