...
class UCTMCTS:
    ...
    def select_child(self, node):
        ...
        for child in node.children.values():
            uct = (child.total_reward / child.visits) + self.c * math.sqrt(math.log(node.visits) / child.visits)
            if uct > best_uct:
                best_uct = uct
                best_child = child
        return best_child
    ...
    def run_simulation(self, root):
        ...
        while node.fully_expanded() and node.children:
            ...
            state, reward, done, afterstate = sim_env.step(node.action)

        # Expansion: if the node has untried actions, expand one.
        if node.untried_actions:
            action = node.untried_actions.pop(0)
            new_state, new_reward, done, afterstate = sim_env.step(action)
            new_node = UCTNode(new_state, new_reward, parent=node, action=action)
